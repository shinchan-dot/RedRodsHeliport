...
namespace SaccFlightAndVehicles
{
    ...
    public class SaccFlightVehicleMenu : UdonSharpBehaviour
    {
        ...
        //** SH-1T **
        public Toggle ThrustVectoringOnGroundToggle;
        private bool ThrustVectoringOnGroundDefault;
        public void ToggleThrustVectoringOnGround()
        {
            bool thrustVectoringOnground = ThrustVectoringOnGroundToggle.isOn;
            foreach (UdonSharpBehaviour SAV in SaccAirVehicles)
            {
                if (SAV)
                {
                    SAV.SetProgramVariable("ThrustVectoringOnGround", thrustVectoringOnground);
                }
            }
        }

        public Toggle LongJoystickToggle;
        private bool LongJoystickDefault;
        public void ToggleLongJoystick()
        {
            bool longJoystick = LongJoystickToggle.isOn;
            foreach(UdonSharpBehaviour SAV in SaccAirVehicles)
            {
                if(SAV)
                {
                    SAV.SetProgramVariable("LongJoystick", longJoystick);
                }
            }
        }
        //** SH-1T **
