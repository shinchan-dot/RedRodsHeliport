...
namespace SaccFlightAndVehicles
{
    ...
    public class SetObjectReferences : Editor, IVRCSDKBuildRequestedCallback
    {
        ...
        public static void SetUpRaceButtons()
        {
            ...
            //** SH-1T **
            var DFUNC_NextRaces = GetAllDFUNC_NextRaces().ToArray();
            var DFUNC_PrevRaces = GetAllDFUNC_PrevRaces().ToArray();
            if (RaceToggleButtons.Length > 0)
            {
                foreach (DFUNC_NextRace DNR in DFUNC_NextRaces)
                {
                    if (!DNR.RaceToggler)
                    {
                        DNR.RaceToggler = RaceToggleButtons[0];
                        PrefabUtility.RecordPrefabInstancePropertyModifications(DNR);
                        EditorUtility.SetDirty(DNR);
                    }
                }
                foreach (DFUNC_PrevRace DPR in DFUNC_PrevRaces)
                {
                    if (!DPR.RaceToggler)
                    {
                        DPR.RaceToggler = RaceToggleButtons[0];
                        PrefabUtility.RecordPrefabInstancePropertyModifications(DPR);
                        EditorUtility.SetDirty(DPR);
                    }
                }
            }
            //** SH-1T **
        ...
        //** SH-1T **
        static List<DFUNC_NextRace> GetAllDFUNC_NextRaces()
        {
            var ls = new List<DFUNC_NextRace>();
            foreach (GameObject g in SceneManager.GetActiveScene().GetRootGameObjects())
            {
                var objs = g.GetComponentsInChildren<DFUNC_NextRace>(true);
                ls.AddRange(objs);
            }
            return ls;
        }
        static List<DFUNC_PrevRace> GetAllDFUNC_PrevRaces()
        {
            var ls = new List<DFUNC_PrevRace>();
            foreach (GameObject g in SceneManager.GetActiveScene().GetRootGameObjects())
            {
                var objs = g.GetComponentsInChildren<DFUNC_PrevRace>(true);
                ls.AddRange(objs);
            }
            return ls;
        }
