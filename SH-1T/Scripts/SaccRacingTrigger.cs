...
namespace SaccFlightAndVehicles
{
    ...
    public class SaccRacingTrigger : UdonSharpBehaviour
    {
        ...
        //** SH-1T **
        public TextMeshProUGUI LastTime_text;
        public TextMeshProUGUI BestTime_text;
        public Text HUDTime_text;
        public Text HUDSplitTime_text;
        public Text HUDLastTime_text;
        public Text HUDBestTime_text;
        //** SH-1T **
        ...
        private void Update()
        {
            if (RaceOn)
            {
                ...
                //** SH-1T **
                if (HUDTime_text) { HUDTime_text.text = SecsToMinsSec(RaceTime); }
                //** SH-1T **
            }
        }
        ...
        private void UpdateSplitTime(float time, bool RaceEnded = false)
        //** SH-1T **
        {
            //** SH-1T **
            //CurrentSplits[CurrentSplit] = (Time.realtimeSinceStartup - RaceStartTime - CalcSubFrameTime());
            CurrentSplits[CurrentSplit] = time;
            //** SH-1T **
            float splitdif;
            if (TrackForward)
            {
                ...
                if (SplitTime_text) { SplitTime_text.text = (!firstrun ? (faster ? "<color=green>" : "<color=red>") : "<color=white>") + SecsToMinsSec(CurrentSplits[CurrentSplit]) + (!firstrun ? (faster ? " (" : " (+") + SecsToMinsSec(splitdif) + ")" : ""); }
                //** SH-1T **
                //** SH-1T **
                if (HUDSplitTime_text) { HUDSplitTime_text.text = SecsToMinsSec(CurrentSplits[CurrentSplit]) + (!firstrun ? (faster ? " (" : " (+") + SecsToMinsSec(splitdif) + ")" : ""); }
                //** SH-1T **
            }
            else
            {
        ...
        public void SetTimerEmpty()
        {
            ...
            //** SH-1T **
            if (HUDTime_text) { HUDTime_text.text = string.Empty; }
            //** SH-1T **
        }
        ...
        public void SetSplitEmpty()
        {
            ...
            //** SH-1T **
            if (HUDSplitTime_text) { HUDSplitTime_text.text = string.Empty; }
            //** SH-1T **
        }
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            //** SH-1T **
            //calculate the time in one place to prevent difference between split and lap
            float time = Time.realtimeSinceStartup - RaceStartTime - CalcSubFrameTime();
            //** SH-1T **
        ...
        {
            string last = CurrentCourse.TimeReporter.MyLastTime > 0 ? SecsToMinsSec(CurrentCourse.TimeReporter.MyLastTime) : "";
            string best = CurrentCourse.MyBestTime > 0 ? SecsToMinsSec(CurrentCourse.MyBestTime) : "";
            if (LastTime_text) { LastTime_text.text = last; }
            if (BestTime_text) { BestTime_text.text = best; }
            if (HUDLastTime_text) { HUDLastTime_text.text = last; }
            if (HUDBestTime_text) { HUDBestTime_text.text = best; }
        }
        private void SetLastBestEmpty()
        {
            if (LastTime_text) { LastTime_text.text = string.Empty; }
            if (BestTime_text) { BestTime_text.text = string.Empty; }
            if (HUDLastTime_text) { HUDLastTime_text.text = string.Empty; }
            if (HUDBestTime_text) { HUDBestTime_text.text = string.Empty; }
        }
        //** SH-1T **
    }
}